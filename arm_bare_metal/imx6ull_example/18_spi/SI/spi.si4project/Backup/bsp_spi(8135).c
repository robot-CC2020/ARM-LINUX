...
#define TRANSFER_BAUDRATE 				10000U /*! Transfer baudrate - 500k */
...
{ 
	...
	unsigned char spitxdata = TxData;

	//ECSPI3->CONREG &= ~(3 << 18);
	ECSPI_SetChannelSelect(ECSPI3, kECSPI_Channel0);

	ECSPI_WriteBlocking(ECSPI3, &spitxdata, 1);
	ECSPI_ReadBlocking(ECSPI3, &spirxdata, 1);

#if 0	
	while((ECSPI3->STATREG & (1 << 0)) == 0) //等待
		ECSPI3->TXDATA = TxData;
	
	while((ECSPI3->STATREG & (1 << 3)) == 0) //等待
		spirxdata = ECSPI3->RXDATA;
#endif
	return spirxdata;
}
#endif
