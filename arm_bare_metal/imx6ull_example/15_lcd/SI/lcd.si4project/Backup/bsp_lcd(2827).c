...
void lcdgpio_init(void)
{
	...
	IOMUXC_SetPinMux(IOMUXC_SNVS_SNVS_TAMPER9_GPIO5_IO09,0);	//复位RESET引脚
	...
	IOMUXC_SetPinConfig(IOMUXC_SNVS_SNVS_TAMPER9_GPIO5_IO09,0xB9);	//复位RESET引脚

	/* GPIO初始化 */
	gpio_config.direction = kGPIO_DigitalOutput;	/* 输出 */
	gpio_config.outputLogic = 1; 					/* 默认高电平，关闭LED */
	gpio_init(GPIO1, 8, &gpio_config);				//背光默认打开
	gpio_init(GPIO5, 9, &gpio_config);				//复位引脚拉高，取消复位

	gpio_pinwrite(GPIO1, 8, 1);	//打开背光
	gpio_pinwrite(GPIO5, 9, 1);	//取消复位
...
/*
 *描述：LCD 时钟初始化
 *      1024*600的屏幕，像素时钟为45MHz，如果希望帧率是60帧的话那么像素时钟为:
 *	 	(600+3+12+20) * (1024+20+160+140) * 60 ≈ 51.2Mhz
 */ 
void lcdclk_init(void)
{
	unsigned char lcdif1prediv = 0;
	unsigned char lcdif1div = 0;

	/* 先初始化video pll 
     * VIDEO PLL = OSC24M * (loopDivider + (denominator / numerator)) / postDivider
     * 将VIDEO_PLL设置为180Mhz，因此loopDivider=30，postDivider=4,不使用小数分频器，因此
     * denominator和numerator设置为0
 	 */
     ...
     * bit[6:0] : 30  设置loopDivider寄存器
	 */
	CCM_ANALOG->PLL_VIDEO =  (2 << 19) | (1 << 13) | (30 << 0); 

	/*
     * MISC2寄存器设置
     * bit[31:30]: 3  VIDEO的post-div设置，时钟源来源于postDivider，4分频
	 */
	CCM_ANALOG->MISC2 &= ~(3 << 30); //先对bit31:30清零
	...
	CCM->CSCDR2 &= ~(7 << 15);  	//清除原来的设置
	CCM->CSCDR2 |= (2 << 15);		//设置LCDIF_PRE_CLK使用PLL5

	/* 设置LCDIF_PRE分频 */
	lcdif1prediv = 4;
	CCM->CSCDR2 &= ~(7 << 12);		//清除原来的设置
	CCM->CSCDR2 |= (lcdif1prediv - 1) << 12;	//设置分频

	/* 设置LCDIF分频 */
	lcdif1div = 1;
	CCM->CBCMR &= ~(7 << 23);		//清除原来的设置
	CCM->CBCMR |= (lcdif1div - 1) << 23;	//1分频
...
void video_pllinit(unsigned char loopdivi, unsigned char postdivi)
{
    ...
    CCM_ANALOG->PLL_VIDEO_DENOM = CCM_ANALOG_PLL_VIDEO_DENOM_B(0);

    pllVideo = CCM_ANALOG_PLL_VIDEO_ENABLE_MASK | CCM_ANALOG_PLL_VIDEO_DIV_SELECT(loopdivi);
	...
	int x,y,dx,dy,Dx,Dy,e,i;
  	...
  	if(dy>dx)
  	{
    	e=-dy;
   		for(i=0;i<dy;i++)
    	{
     		lcd_drawpoint(x,y,tftlcd_dev.forecolor);//画点函数
     		if(Dy>=0)y++;  
     		else y--;   
     		e+=2*dx;
     		if(e>=0)
...
{  							  
	...
	{   
		...
		{			    
			...
			else lcd_drawpoint(x,y,tftlcd_dev.backcolor);
			temp<<=1;
			y++;
