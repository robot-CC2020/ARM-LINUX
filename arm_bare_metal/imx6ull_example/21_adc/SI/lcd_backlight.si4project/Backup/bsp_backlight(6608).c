...
void pwm1_irqhandler(void)
{
	...
	{
		pwm1_setduty(backlight_dev.pwm_duty); /* 将占空比信息写入到FIFO中
											   * 其实就是设置占空比 
											   */
...
void backlight_init(void)
{
	...
	/*
   	 ...
   	 * bit[21]		: 0   关闭字节交换
   	 * bit[20]		: 0	  关闭半字数据交换
   	 * bit[19:18]	: 00  PWM输出引脚在计数器溢出从新计数的时候输出高电平
   	 *					  在计数器计数值达到比较值以后输出低电平
	...
	/* 设置PWM周期 */
	pwm1_setperiod_value(1000);	/* 周期寄存器设置为1000，那么PWM频率就是
								 * 1M/1000 = 1KHz。
								 */
