...
void imx6u_clkinit(void)
{
	...
	/* 3、设置AHB时钟 最小6Mhz， 最大132Mhz (boot rom自动设置好了)*/
	
	/* 4、设置IPG_CLK_ROOT最小3Mhz，最大66Mhz (boot rom自动设置好了)*/
	
	/* 5、设置PERCLK_CLK_ROOT时钟 */
	reg = CCM->CSCMR1;
	reg &= ~(1 << 6);							/* 先清零,清零以后PERCLK时钟源就选择了IPG   				 */
	CCM->CSCMR1 = reg;							/* 写入寄存器									 */	
}
