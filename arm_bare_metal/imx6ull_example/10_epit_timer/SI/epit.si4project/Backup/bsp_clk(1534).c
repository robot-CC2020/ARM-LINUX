...
void imx6u_clkinit(void)
{
	...
	CCM->CBCMR &= ~(3 << 18); 	/* pre_periph_clk=PLL2=528MHz*/ 
	 ...
	 * AHB_PODF默认就是4分频，所以即使我们不设置AHB_PODF，
	 * AHB_ROOT_CLK也依旧等于528/4=132Mhz。
	 */
#if 0
	/* 要先关闭AHB_ROOT_CLK输出，否则时钟设置会出错 */
	CCM->CBCDR &= ~(7 << 10);	/* CBCDR的AHB_PODF清零 */
	CCM->CBCDR |= 3 << 10;		/* AHB_PODF 4分频，AHB_CLK_ROOT=132MHz */
